...
# Use the same l_total_data and L_h logic to ensure balance
l_h_fixed <- sum(L_mat)
...
varlist <- c(
    varlist,
    get_flow_values(rep(1, 3), "p", sam_prod),
    get_flow_values(rep(0, 3), "pi", acts),
...
listcalib <- c(
    get_flow_values(rep(0.2, 3), "alpha", sam_prod),
