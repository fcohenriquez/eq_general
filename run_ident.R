...
# gEcon con Cobb-Douglas no permite ceros en los insumos si p > 0.
...
# Tasa de IVA efectiva (Basada en toda la demanda final gravada)
# La base es D + G + I + Var_Exis
total_final_demand_market <- D_market + GG_market + I_market + as.numeric(chile_sam[sam_p, "Var_Exis"])
...
# --- 3.4. Cuentas del Gobierno (GOB) ---
K_g_val <- chile_sam["Gobierno", "Capital"]
CSg_val <- chile_sam["Gobierno", "Cont_soc"]
PSg_val <- chile_sam["Pres_soc", "Gobierno"]

# Impuestos Indirectos (Consistentes con GCN)
vat_consumption <- sum(vat_rates * (D_vals + GG_vals + I_vals + VAR_EXIS_vals))
vat_production <- sum(t_iva_prod_rates * V_sector_vals)
tariff_total <- sum(tariff_vals)
other_ind_taxes <- sum((t_imp_prod_rates + t_imp_espec_rates) * V_sector_vals)

# Impuestos Directos Totales de la SAM
TOTAL_DIR_TAX_SAM <- sum(as.numeric(chile_sam["Imp_dir", ]))
OTH_DIR_TAX_val <- TOTAL_DIR_TAX_SAM - DIR_TAX_H_val - FIT_val

TOTAL_TAX_SAM <- vat_consumption + vat_production + tariff_total + other_ind_taxes + TOTAL_DIR_TAX_SAM

ING_GOB_val <- K_g_val + CSg_val + TOTAL_TAX_SAM
# Nota: La Eq de INC en gEcon debe incluir OTH_DIR_TAX_val para que Hogares balancee a 142.48
# En la SAM, pareciera que OTH_DIR_TAX lo paga el Capital o las Empresas de forma indirecta.
# Lo restaremos de la renta del consumidor para cerrar el sistema.

# Nota: No calculamos SAV_G_val aquí todavía. Lo usamos para cerrar Walras.

# --- 3.5. Cierre del Modelo (Balanza de Pagos y Walras) ---
# 1. El Ahorro Externo (SAVrow) DEBE ser el déficit de cuenta corriente (BOP)
SAV_ROW_val <- (sum(M_market) + fact_row_out_k_val + fact_row_out_l_val) -
    (sum(E_market) + fact_row_in_k_val + fact_row_in_l_val)

# 2. El Ahorro del Gobierno (SAVg) actúa como el residual final para cerrar Walras
# inversión_total = SAVh + SAVf + SAVg + SAVrow
total_inv_market <- sum((I_vals + VAR_EXIS_vals) * (1 + vat_rates))
SAV_G_val <- total_inv_market - (SAV_H_val + SAV_F_val + SAV_ROW_val)

# 3. Verificación de Consistencia del Gobierno
# INGG = Gastos_Consumo_G + TR_G + PS_G + SAV_G
cat("--- Verificación Final de Balance Institucional ---\n")
cat("Ahorro Externo (BOP):       ", SAV_ROW_val, "\n")
cat("Ahorro Gobierno (Residual): ", SAV_G_val, "\n")
cat("Balance Gobierno (INGG - GTg):", ING_GOB_val - (sum(GG_market) + TR_GOV_val + PSg_val + SAV_G_val), "\n")
cat("-----------------------------------------------\n")
...
model <- set_free_par(model, c(
    ...
    fit_data = FIT_val,
    oth_dir_tax_data = OTH_DIR_TAX_val,
...
varlist <- list(
    ...
    FIRMTAX = FIT_val, OTH_DIR_TAX = OTH_DIR_TAX_val, Tax = TOTAL_TAX_SAM,
