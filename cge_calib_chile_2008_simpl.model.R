...
# info
info__ <- c("cge_calib_chile_2008_simpl", "D:/francisco.henriquez/Mis documentos/proyectos/equilibrio_general/eq_general/cge_calib_chile_2008_simpl.gcn", "2026-02-25 10:31:04", "false")
...
equations__ <- c("-1 + beta_l<'Prod','Act'> * gamma<'Prod','Act'> * p<'Prod'> * K<'Prod','Act'>^beta_k<'Prod','Act'> * L<'Prod','Act'>^(-1 + beta_l<'Prod','Act'>) = 0",
                 ...
                 "SAV + SAV_F + SAV_G - p<'Prod'> * I<'Prod'> = 0",
...
vareqmap__ <- sparseMatrix(i = c(1, 1, 1, 2, 3, 4, 4, 5, 5, 5,
                           ...
                           j = c(26, 32, 33, 1, 13, 26, 30, 1, 26, 32,
                                 ...
                                 8, 19, 8, 22, 9, 23, 24, 28, 25, 26,
                                 28, 30, 31, 26, 28, 29, 30, 31, 26, 34,
                                 35, 32, 33, 35, 1, 10, 13, 14, 15, 16,
                                 17, 2, 7, 14, 27, 3, 7, 18, 26, 28,
                                 1, 5, 9, 12, 21, 18, 19, 20, 26, 31,
                                 1, 11, 14, 21, 25, 1, 26, 27, 32, 33,
                                 35, 6, 17, 20, 23, 26, 30, 1, 8, 11,
...
freepareqmap__ <- sparseMatrix(i = c(3, 4, 6, 7, 8, 10, 10, 11, 11, 12,
                                     ...
                                     27, 31, 31, 34),
                               j = c(3, 20, 10, 14, 17, 2, 5, 2, 6, 2,
                                     7, 15, 16, 9, 13, 11, 12, 4, 22, 22,
                                     8, 1, 8, 1),
                               x = rep(1, 24), dims = c(35, 22))
...
ss_eq__ <- function(v, pc, pf)
{
    ...
    r[18] = -v[22] + pf[11] * v[8]
    r[19] = -v[23] + pf[12] * v[9]
    r[20] = v[24] - (pc[1] * v[28]^(pf[4]^-1 * (-1 + pf[4])))^(pf[4] * (-1 + pf[4])^-1)
    r[21] = -v[25] + pf[22] * v[26] * (v[28] + v[30] + v[31])
    r[22] = -v[29] + v[26] * (v[28] + v[30] + v[31])
    r[23] = -v[34] + v[26] * v[35] * (1 + pf[22])
    r[24] = -v[35] + pc[4] * v[32]^pc[2] * v[33]^pc[3]
    r[25] = v[13] - v[14] + v[1] * v[10]
    r[26] = -v[15] + v[16] + v[17]
    r[27] = v[2] - v[7] + v[27] - pf[8] * v[14]
    r[28] = v[3] - v[7] + v[18] + v[26] * v[28]
    r[29] = v[5] - v[9] + v[21] + v[1] * v[12]
    r[30] = v[18] + v[19] + v[20] - v[26] * v[31]
    r[31] = -v[21] + v[25] + pf[8] * v[14] + pf[1] * v[1] * v[11]
    r[32] = v[27] + v[33] + v[1] * v[32] - v[26] * v[35]
    r[33] = -v[6] + v[17] + v[20] + v[23] + v[26] * v[30]
...
ss_calibr_eq_jacob__ <- function(v, pc, pf)
{
    ...
    jac[42] = -1
    jac[43] = 1
    jac[44] = -pc[1] * v[28]^(-1 + pf[4]^-1 * (-1 + pf[4])) * (pc[1] * v[28]^(pf[4]^-1 * (-1 + pf[4])))^(-1 + pf[4] * (-1 + pf[4])^-1)
    jac[45] = -pf[4] * (-1 + pf[4])^-1 * v[28]^(pf[4]^-1 * (-1 + pf[4])) * (pc[1] * v[28]^(pf[4]^-1 * (-1 + pf[4])))^(-1 + pf[4] * (-1 + pf[4])^-1)
    jac[46] = -1
    jac[47] = pf[22] * (v[28] + v[30] + v[31])
    jac[48] = pf[22] * v[26]
    jac[49] = pf[22] * v[26]
    jac[50] = pf[22] * v[26]
    jac[51] = v[28] + v[30] + v[31]
    jac[52] = v[26]
    jac[53] = -1
    jac[54] = v[26]
    jac[55] = v[26]
    ...
    jac[57] = -1
    jac[58] = v[26] * (1 + pf[22])
    jac[59] = pc[2] * pc[4] * v[32]^(-1 + pc[2]) * v[33]^pc[3]
    jac[60] = pc[3] * pc[4] * v[32]^pc[2] * v[33]^(-1 + pc[3])
    jac[61] = -1
    jac[62] = pc[4] * log(v[32]) * v[32]^pc[2] * v[33]^pc[3]
    jac[63] = pc[4] * log(v[33]) * v[32]^pc[2] * v[33]^pc[3]
    jac[64] = v[32]^pc[2] * v[33]^pc[3]
    jac[65] = v[10]
    jac[66] = v[1]
    jac[67] = 1
    jac[68] = -1
    ...
    jacob <- sparseMatrix(i = c(1, 1, 1, 1, 1, 1, 2, 3, 4, 4,
                          ...
                          j = c(26, 32, 33, 37, 38, 39, 1, 13, 26, 30,
                                ...
                                8, 16, 9, 17, 7, 18, 8, 19, 8, 22,
                                9, 23, 24, 28, 36, 25, 26, 28, 30, 31,
                                26, 28, 29, 30, 31, 26, 34, 35, 32, 33,
                                35, 37, 38, 39, 1, 10, 13, 14, 15, 16,
                                17, 2, 7, 14, 27, 3, 7, 18, 26, 28,
                                1, 5, 9, 12, 21, 18, 19, 20, 26, 31,
                                1, 11, 14, 21, 25, 1, 26, 27, 32, 33,
                                35, 6, 17, 20, 23, 26, 30, 1, 8, 11,
