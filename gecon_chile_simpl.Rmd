...
listcalib <- c(get_flow_values(rep(0.2, n), "alpha", sam_prod),
    get_flow_values(matrix(0.5635, n, m), "beta_k", sam_prod, acts),
    get_flow_values(matrix(0.43654, n, m), "beta_l", sam_prod, acts),
    gamma__Prod__Act = g_p
)
```

```{r}
flow1["Hogares", "Trabajo"]+flow1["Hogares", "Capital"]
```
...
```
```{r}
list_eq(model, eq_idx = c(23, 27, 20))
```
```{r}
