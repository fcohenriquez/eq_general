...
# 2. SHOCK: 10% VAT Increase
```{r}
vat_rates_shock <- vat_rates * 1.1

model_shock <- set_free_par(model, get_flow_values(vat_rates_shock, "vat", p_names))

model_shock <- steady_state(model_shock, calibration = F) # Simulation, not calibration

res_shock <- get_ss_values(model_shock)
...
comp <- data.frame(
    ...
    Shock = as.numeric(res_shock[vars_to_compare])

)

comp$Diff_Pct <- (comp$Shock / comp$Base - 1) * 100
