...
ss_vals <- get_ss_values(model)
p_eq <- ss_vals[paste0("p__", p_names)]
pm_eq <- ss_vals[paste0("pm__", p_names)]
pe_eq <- ss_vals[paste0("pe__", p_names)]

C_model <- sum(ss_vals[paste0("D__", p_names)] * p_eq * (1 + vat_r))
G_model <- sum(ss_vals[paste0("G__", p_names)] * p_eq * (1 + vat_r))
I_model <- sum(ss_vals[paste0("I__", p_names)] * p_eq * (1 + vat_r))
V_model <- sum(ss_vals[paste0("VarExis__", p_names)] * p_eq * (1 + vat_r))
X_model <- sum(ss_vals[paste0("Exports__", p_names)] * pe_eq)
M_model <- sum(ss_vals[paste0("Imports__", p_names)] * pm_eq)

pib_model <- C_model + G_model + I_model + V_model + X_model - M_model
...
pib_df <- data.frame(
    ...
    Modelo_Eq = c(C_model, G_model, I_model, V_model, X_model, M_model, pib_model)
)
