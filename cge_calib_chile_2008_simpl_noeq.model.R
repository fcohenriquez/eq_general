...
# info
info__ <- c("cge_calib_chile_2008_simpl_noeq", "D:/francisco.henriquez/Mis documentos/proyectos/equilibrio_general/eq_general/cge_calib_chile_2008_simpl_noeq.gcn", "2026-02-23 17:04:04", "false")
...
vareqmap__ <- sparseMatrix(i = c(1, 1, 1, 2, 3, 3, 4, 4, 4, 4,
                                 ...
                                 36, 36, 36, 37),
                           j = c(28, 34, 35, 14, 28, 32, 1, 28, 34, 35,
                                 3, 8, 3, 4, 3, 5, 7, 10, 11, 12,
                                 13, 9, 17, 10, 18, 8, 19, 9, 20, 21,
                                 28, 33, 10, 22, 9, 24, 10, 25, 26, 30,
                                 27, 28, 30, 32, 33, 28, 30, 31, 32, 33,
                                 28, 36, 37, 34, 35, 37, 1, 6, 12, 15,
                                 6, 23, 27, 1, 11, 14, 15, 16, 17, 18,
                                 2, 8, 15, 29, 3, 8, 19, 28, 30, 1,
                                 5, 10, 13, 23, 19, 20, 21, 22, 1, 28,
                                 29, 34, 35, 37, 7, 18, 22, 25, 28, 32,
                                 1, 9, 12, 17, 20, 24, 1, 2, 11, 14,
                                 16, 24, 25, 30),
...
calibrpareqmap__ <- sparseMatrix(i = c(1, 1, 1, 4, 4, 4, 20, 24, 24, 24,
                                       37),
                                 j = c(2, 3, 4, 2, 3, 4, 1, 2, 3, 4,
                                       1),
                                 x = rep(1, 11), dims = c(37, 4))
...
freepareqmap__ <- sparseMatrix(i = c(2, 3, 5, 6, 7, 9, 9, 10, 10, 11,
                                     11, 12, 13, 14, 15, 17, 18, 19, 20, 21,
                                     23, 25, 25, 29, 35, 37),
                               j = c(3, 21, 10, 14, 17, 2, 5, 2, 6, 2,
                                     7, 15, 16, 9, 13, 18, 11, 12, 4, 23,
                                     23, 1, 8, 8, 1, 4),
                               x = rep(1, 26), dims = c(37, 23))
...
{
    r <- numeric(37)
    r[1] = -1 + pc[3] * pc[4] * v[28] * v[34]^pc[2] * v[35]^(-1 + pc[3])
    r[2] = pf[3] - v[14]
    r[3] = pf[21] - v[28] * v[32]
    r[4] = -v[1] + pc[2] * pc[4] * v[28] * v[34]^(-1 + pc[2]) * v[35]^pc[3]
    r[5] = -v[3] + pf[10] * v[8]
    r[6] = -v[4] + pf[14] * v[3]
    r[7] = -v[5] + pf[17] * v[3]
    r[8] = v[7] - v[10]
    ...
    r[11] = -v[13] + pf[2] * pf[7]
    r[12] = -v[17] + pf[15] * v[9]
    r[13] = -v[18] + pf[16] * v[10]
    r[14] = -v[19] + pf[9] * v[8]
    r[15] = -v[20] + pf[13] * v[9]
    r[16] = -v[21] + v[28] * v[33]
    r[17] = -v[22] + pf[18] * v[10]
    r[18] = -v[24] + pf[11] * v[9]
    r[19] = -v[25] + pf[12] * v[10]
    r[20] = v[26] - (pc[1] * v[30]^(pf[4]^-1 * (-1 + pf[4])))^(pf[4] * (-1 + pf[4])^-1)
    r[21] = -v[27] + pf[23] * v[28] * (v[30] + v[32] + v[33])
    r[22] = -v[31] + v[28] * (v[30] + v[32] + v[33])
    r[23] = -v[36] + v[28] * v[37] * (1 + pf[23])
    r[24] = -v[37] + pc[4] * v[34]^pc[2] * v[35]^pc[3]
    ...
    r[33] = v[29] + v[35] + v[1] * v[34] - v[28] * v[37]
    r[34] = -v[7] + v[18] + v[22] + v[25] + v[28] * v[32]
    r[35] = -v[9] + v[17] + v[20] + v[24] + pf[1] * v[1] * v[12]
    r[36] = -v[2] + v[14] + v[16] + v[24] + v[25] + v[1] * v[11]
    r[37] = pc[1] * v[30]^(-1 + pf[4]^-1 * (-1 + pf[4])) * (pc[1] * v[30]^(pf[4]^-1 * (-1 + pf[4])))^(-1 + pf[4] * (-1 + pf[4])^-1)
...
ss_calibr_eq_jacob__ <- function(v, pc, pf)
{
    r <- numeric(4)
    jac <- numeric(130)
    jac[1] = pc[3] * pc[4] * v[34]^pc[2] * v[35]^(-1 + pc[3])
    jac[2] = pc[2] * pc[3] * pc[4] * v[28] * v[34]^(-1 + pc[2]) * v[35]^(-1 + pc[3])
    jac[3] = pc[3] * pc[4] * v[28] * (-1 + pc[3]) * v[34]^pc[2] * v[35]^(-2 + pc[3])
    jac[4] = pc[3] * pc[4] * v[28] * log(v[34]) * v[34]^pc[2] * v[35]^(-1 + pc[3])
    jac[5] = pc[4] * v[28] * v[34]^pc[2] * v[35]^(-1 + pc[3]) + pc[3] * pc[4] * v[28] * log(v[35]) * v[34]^pc[2] * v[35]^(-1 + pc[3])
    jac[6] = pc[3] * v[28] * v[34]^pc[2] * v[35]^(-1 + pc[3])
    jac[7] = -1
    jac[8] = -v[32]
    jac[9] = -v[28]
    jac[10] = -1
    jac[11] = pc[2] * pc[4] * v[34]^(-1 + pc[2]) * v[35]^pc[3]
    jac[12] = pc[2] * pc[4] * v[28] * (-1 + pc[2]) * v[34]^(-2 + pc[2]) * v[35]^pc[3]
    jac[13] = pc[2] * pc[3] * pc[4] * v[28] * v[34]^(-1 + pc[2]) * v[35]^(-1 + pc[3])
    jac[14] = pc[4] * v[28] * v[34]^(-1 + pc[2]) * v[35]^pc[3] + pc[2] * pc[4] * v[28] * log(v[34]) * v[34]^(-1 + pc[2]) * v[35]^pc[3]
    jac[15] = pc[2] * pc[4] * v[28] * log(v[35]) * v[34]^(-1 + pc[2]) * v[35]^pc[3]
    jac[16] = pc[2] * v[28] * v[34]^(-1 + pc[2]) * v[35]^pc[3]
    jac[17] = -1
    jac[18] = pf[10]
    jac[19] = pf[14]
    jac[20] = -1
    ...
    jac[44] = -1
    jac[45] = 1
    jac[46] = -pc[1] * v[30]^(-1 + pf[4]^-1 * (-1 + pf[4])) * (pc[1] * v[30]^(pf[4]^-1 * (-1 + pf[4])))^(-1 + pf[4] * (-1 + pf[4])^-1)
    jac[47] = -pf[4] * (-1 + pf[4])^-1 * v[30]^(pf[4]^-1 * (-1 + pf[4])) * (pc[1] * v[30]^(pf[4]^-1 * (-1 + pf[4])))^(-1 + pf[4] * (-1 + pf[4])^-1)
    jac[48] = -1
    jac[49] = pf[23] * (v[30] + v[32] + v[33])
    jac[50] = pf[23] * v[28]
    jac[51] = pf[23] * v[28]
    jac[52] = pf[23] * v[28]
    jac[53] = v[30] + v[32] + v[33]
    jac[54] = v[28]
    jac[55] = -1
    jac[56] = v[28]
    jac[57] = v[28]
    ...
    jac[59] = -1
    jac[60] = v[28] * (1 + pf[23])
    jac[61] = pc[2] * pc[4] * v[34]^(-1 + pc[2]) * v[35]^pc[3]
    jac[62] = pc[3] * pc[4] * v[34]^pc[2] * v[35]^(-1 + pc[3])
    jac[63] = -1
    jac[64] = pc[4] * log(v[34]) * v[34]^pc[2] * v[35]^pc[3]
    jac[65] = pc[4] * log(v[35]) * v[34]^pc[2] * v[35]^pc[3]
    jac[66] = v[34]^pc[2] * v[35]^pc[3]
    jac[67] = pf[1] * v[12]
    jac[68] = -1
    jac[69] = pf[1] * v[1]
    jac[70] = pf[8]
    ...
    jac[122] = 1
    jac[123] = 1
    jac[124] = pc[1] * (-1 + pf[4]^-1 * (-1 + pf[4])) * v[30]^(-2 + pf[4]^-1 * (-1 + pf[4])) * (pc[1] * v[30]^(pf[4]^-1 * (-1 + pf[4])))^(-1 + pf[4] * (-1 + pf[4])^-1) + pf[4]^-1 * pc[1]^2 * (-1 + pf[4]) * (-1 + pf[4] * (-1 + pf[4])^-1) * (v[30]^(-1 + pf[4]^-1 * (-1 + pf[4])))^2 * (pc[1] * v[30]^(pf[4]^-1 * (-1 + pf[4])))^(-2 + pf[4] * (-1 + pf[4])^-1)
    jac[125] = v[30]^(-1 + pf[4]^-1 * (-1 + pf[4])) * (pc[1] * v[30]^(pf[4]^-1 * (-1 + pf[4])))^(-1 + pf[4] * (-1 + pf[4])^-1) + pc[1] * (-1 + pf[4] * (-1 + pf[4])^-1) * v[30]^(-1 + 2 * pf[4]^-1 * (-1 + pf[4])) * (pc[1] * v[30]^(pf[4]^-1 * (-1 + pf[4])))^(-2 + pf[4] * (-1 + pf[4])^-1)
    jac[126] = 1
    jac[127] = 1
    jac[128] = 1
    jac[129] = 1
    jac[130] = 1
    jacob <- sparseMatrix(i = c(1, 1, 1, 1, 1, 1, 2, 3, 3, 4,
                                4, 4, 4, 4, 4, 4, 5, 5, 6, 6,
                                ...
                                33, 33, 33, 33, 34, 34, 34, 34, 34, 34,
                                35, 35, 35, 35, 35, 35, 36, 36, 36, 36,
                                36, 36, 36, 37, 37, 38, 39, 40, 41, 41),
                          j = c(28, 34, 35, 39, 40, 41, 14, 28, 32, 1,
                                28, 34, 35, 39, 40, 41, 3, 8, 3, 4,
                                3, 5, 7, 10, 11, 12, 13, 9, 17, 10,
                                18, 8, 19, 9, 20, 21, 28, 33, 10, 22,
                                9, 24, 10, 25, 26, 30, 38, 27, 28, 30,
                                32, 33, 28, 30, 31, 32, 33, 28, 36, 37,
                                34, 35, 37, 39, 40, 41, 1, 6, 12, 15,
                                6, 23, 27, 1, 11, 14, 15, 16, 17, 18,
                                2, 8, 15, 29, 3, 8, 19, 28, 30, 1,
                                5, 10, 13, 23, 19, 20, 21, 22, 1, 28,
                                29, 34, 35, 37, 7, 18, 22, 25, 28, 32,
                                1, 9, 12, 17, 20, 24, 1, 2, 11, 14,
                                16, 24, 25, 30, 38, 30, 37, 35, 39, 40),
                          x = jac, dims = c(41, 41))
