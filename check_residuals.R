...
# This script loads the model with my changes and fixed R logic,
# then prints the residuals before calibration.
...
tm_r <- TM_m / (D_m + G_m + I_m + V_m + E_m + rowSums(X_matrix) + pad_floor)
tm_r["prod_6"] <- 0 
vat_r <- as.numeric(chile_sam["IVA", p_names]) / (D_m + G_m + I_m + V_m + pad_floor)

D_v <- D_m / (1 + vat_r + tm_r)
...
# Cost of intermediates per sector
cost_int <- colSums(X_matrix) 
# Value added per sector (at factor cost)
va_vals <- pmax(net_rev - cost_int, pad_floor)
...
gamma_v <- numeric(12)
for (i in 1:12) {
    # gamma for the VA bundle part
    gamma_v[i] <- V_sector_vals[i] / ( (K_vals[i]^bk[i]) * (L_vals[i]^bl[i]) + pad_floor )
}

omega_val <- 0.5
alpha_v_unsc <- (1 + vat_r + tm_r) * (D_v + 1e-8)^(1/omega_val)
...
par_flat <- unlist(list(
    ...
    get_flow_values(vat_r, "vat", p_names), get_flow_values(Ar_m / (M_m + pad_floor), "tar", p_names),
    ...
    get_flow_values(G_m, "gg_market_data_p", p_names), get_flow_values(V_m[1:12], "vexist", pmc_names),
    get_flow_values(E_m, "edp", p_names), get_flow_values(M_m, "mdp", p_names), get_flow_values(I_m[1:12], "I_data", pmc_names),
...
init_vars <- unlist(list(
    var_list,
    get_flow_values(rep(1, 12), "pi", s_names), get_flow_values(rep(1, 13), "p", p_names),
    get_flow_values(rep(1, 13), "pe", p_names), get_flow_values(1 / (1 + Ar_m / (M_m + pad_floor)), "pm", p_names),
    get_flow_values(pmax(D_v, 1e-8), "D", p_names), get_flow_values(pmax(V_sector_vals, 1e-8), "V", s_names),
    get_flow_values(pmax(V_sector_vals, 1e-8), "V_VA", s_names), get_flow_values(pmax(V_sector_vals, 1e-8), "V_INT", s_names),
...
names(res) <- model@equations$equation
print(res[abs(res) > 1e-5])
print(pracma::Norm(res))
