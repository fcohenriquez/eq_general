...
Se va a modelar una economía en que están los elementos domésticos (todos los impuestos y variación de existencias.
...
chile_sam <- read.csv("macro_sam_2008_dom1.csv", row.names = 1, check.names = FALSE)
...
model <- make_model("cge_calib_chile_domestico1.gcn")
...
V_vals <- as.numeric(rowSums(make_matrix)) # Output total por actividad
make_shares <- make_matrix / V_vals
...
# Variación de existencias (Market price)
...
# Cálculo de tasas de IVA sobre demanda (basadas en valores netos)
# El IVA en la SAM (fila IVA, columnas Prod *) se aplica sobre D, GG, I, X y Var_Exis
vat_vals <- as.numeric(chile_sam["IVA", sam_p])
total_demand_market <- D_market + GG_market + I_market + VAR_EXIS_market + rowSums(X_matrix)
vat_rates <- vat_vals / (total_demand_market - vat_vals)
vat_rates[is.nan(vat_rates)] <- 0

# Nuevas tasas de impuestos sobre la producción (Actividades)
t_imp_prod_rates <- as.numeric(chile_sam["Imp_prod", sam_a]) / V_vals
t_imp_espec_rates <- as.numeric(chile_sam["Imptos_Espec", sam_a]) / V_vals
t_iva_prod_rates <- as.numeric(chile_sam["IVA", sam_a]) / V_vals
...
model <- set_free_par(model, c(
    ...
    get_flow_values(V_vals, "v_data", p_names),
...
varlist <- list(
    ...
    FIRM_TAX = FIT_val,
    IMP_PROD_REC = chile_sam["Imp_prod", "Total"],
    IMP_ESPEC_REC = chile_sam["Imptos_Espec", "Total"],
    TOTAL_TAX = TOTAL_TAX_SAM,
    VAT = chile_sam["IVA", "Total"], WALRAS = 0
)
...
varlist <- c(
    ...
    get_flow_values(VAR_EXIS_vals, "VAR_EXIS", p_names),
    get_flow_values(V_vals, "V", p_names),
    get_flow_values(make_matrix, "Y_ap", p_names, p_names),
    get_flow_values(X_vals, "X", p_names, p_names),
    get_flow_values(V_vals, "revenue", p_names),
    get_flow_values(colSums(X_vals * vat_rates), "intermediate_vat", p_names),
...
V_net_prod <- V_vals * (1 - t_prod_total)

beta_l_vals <- L_vals / V_net_prod
# Column sums of X_vals (Intermediate costs at net prices + VAT on products)
# Wait, in GOC identity it's SUM(X * p * (1+vat)).
# So the cost share is (X * (1+vat)) / V_net_prod = X_market / V_net_prod
X_market <- X_vals * (1 + vat_rates) # Reconstructing market value (X_matrix)
beta_x_vals <- sweep(X_market, 2, V_net_prod, "/")
...
gamma_vals <- V_vals / (K_vals^beta_k_vals * L_vals^beta_l_vals * exp(term_X_val))
...
# ANALYSIS IVA
```{r}
vars_to_compare <- c("VAT", "PI_TAX", "FIRM_TAX", "INC", "TOTAL_TAX", "U", "V__Manu", "V__Prim", "V__Serv", "D__Manu", "D__Prim", "D__Serv")
