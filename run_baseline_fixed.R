...
pmc_names <- paste0("prod_", 1:12) # FIX: Use 12 for market-clearing only parameters
...
par_flat <- unlist(list(
    ...
    get_flow_values(V_m[1:12], "vexist", pmc_names), # FIX: Use only 12
    get_flow_values(E_m, "edp", p_names), 
    get_flow_values(M_m, "mdp", p_names), 
    get_flow_values(I_m[1:12], "I_data", pmc_names), # FIX: Use only 12
...
cat("Iniciando steady_state baseline\n")
model <- steady_state(model, calibration = TRUE)
cat("Norma final de residuos baseline:", pracma::Norm(model@residual_vector), "\n")
