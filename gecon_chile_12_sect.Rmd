...
# Rates matching GCN identities: Layer / Total Demand
tm_r <- TM_m / (Demand_m + pad_floor)
tm_r[6] <- 0  # <---- Commerce sector supplies margins, it doesn't pay a negative margin rate on itself
is_margin_good <- rep(0, 13)
is_margin_good[6] <- 1
...
# Se utiliza calibration = TRUE para permitir la inyección de parámetros vía initval_calibr_par
model <- steady_state(model, calibration = TRUE)

cat("Norma final de residuos:", pracma::Norm(model@residual_vector), "\n")
cat("Modelo finalizado y listo para var_info().\n")
```
