...
```{r sam_load}
chile_sam_raw <- read.csv("../data/sam_2023_6sect_sinmg.csv", check.names = FALSE, stringsAsFactors = FALSE)
...
# Factor shares for Leontief-CD nested production (crude at pz=1, refined later with p_base)
...
```{r model_gen}
model <- make_model("../script/cge_calib_chile_6_sect_23.gcn")
...
# Match sectoral shares to NOM_I_BASE (which excludes stocks)
I_base_benchmark <- sum(I_m)
sh_i_h_v <- SAV_h_v / I_base_benchmark
sh_i_f_v <- SAVf_v / I_base_benchmark
sh_i_g_v <- (I_base_benchmark - SAV_h_v - SAVf_v) / I_base_benchmark
...
sigma_q_val <- 2    # Armington elasticity of substitution (imports vs domestic) Original 2.0
sigma_t_val <- 2    # CET elasticity of transformation (exports vs domestic) original 2.0
...
# Calibration with Leontief intermediates and CD in primary factors (K, L only)
net_rev <- revenue_vals * (1.0 - t_pro_r - t_esp_r)

# Calculate producer activity price and physical output
...
# --- Leontief io coefficients: io_{i,s} = X_phys_{i,s} / V_phys_s ---
io_matrix <- sweep(X_phys, 2, V_phys + pad_floor, "/")
io_matrix[5, sa_idx] <- 0        # prod_const not used by SA
io_matrix[5, sc_idx] <- 0        # prod_const not used by SC
io_matrix <- pmax(io_matrix, 0)

# VA_nom = net_revenue - intermediate input cost at base prices
# Note: SUM_i(io_{i,s} * V_phys_s * p_base_i) = SUM_i(X_phys_{i,s} * p_base_i) = colSums(X_matrix)
X_nom_by_sector <- colSums(sweep(X_phys, 1, p_base, "*"))
VA_nom <- pmax(net_rev - X_nom_by_sector, pad_floor)

# Recalibrate bk, bl as shares of VA_nom (bk + bl = 1 at benchmark)
bk <- K_vals / (VA_nom + pad_floor)
bl <- L_vals / (VA_nom + pad_floor)

cat(sprintf("bk + bl check (should be ~1): %s\n",
    paste(round(bk + bl, 4), collapse = ", ")))

# Gamma for CD in primary factors only (no X in production function)
gamma_v <- numeric(6)
for (i in 1:6) {
...
par_flat <- unlist(list(
    ...
    # Leontief io coefficients (free parameters passed directly to the model)
    get_flow_values(io_matrix[p5_idx, sa_idx, drop=FALSE], "io", p_names[p5_idx], s_names[sa_idx]),
    get_flow_values(io_matrix[, sb_idx, drop=FALSE],        "io", p_names,          s_names[sb_idx]),
    get_flow_values(io_matrix[p5_idx, sc_idx, drop=FALSE], "io", p_names[p5_idx], s_names[sc_idx]),
    # Armington/CET parameters
    sigma_q = sigma_q_val, sigma_t = sigma_t_val,
    get_flow_values(gamma_e_v[c(1, 2, 3, 4, 6)], "gamma_e", p_names[c(1, 2, 3, 4, 6)]),
    get_flow_values(at_v[c(1, 2, 3, 4, 6)], "at", p_names[c(1, 2, 3, 4, 6)]),
...
init_vars <- unlist(list(
    ...
    NOM_I_BASE = sum(I_m),
    TOTAL_INVEST = Nominal_I_Total,
    inv_adj_h = 1.0, inv_adj_f = 1.0, inv_adj_g = 1.0,
    ...
    get_flow_values(X_matrix[p5_idx, sc_idx, drop=FALSE] / p_base[p5_idx], "X", p5_names, sc_names),
    get_flow_values(VA_nom[sa_idx], "va_nom", s_names[sa_idx]),
    get_flow_values(VA_nom[sb_idx], "va_nom", s_names[sb_idx]),
    get_flow_values(VA_nom[sc_idx], "va_nom", s_names[sc_idx]),
