...
for(i in 2:ncol(chile_sam)) chile_sam[[i]] <- as.numeric(as.character(chile_sam[[i]]))
...
I_m[which(p_names=="prod_const")] <- I_m[which(p_names=="prod_const")] + sum(dropped_x)
V_m <- as.numeric(chile_sam[p_names,"Var_Exis"])
M_m <- as.numeric(chile_sam["row", p_names]); names(M_m) <- p_names
E_m <- as.numeric(chile_sam[p_names,"row"]); names(E_m) <- p_names
...
L_vals <- pmax(L_vals, 0.1); K_vals <- pmax(K_vals, 0.1)
Kh_val <- chile_sam["Hogares","Capital"]; Lh_val <- chile_sam["Hogares","Trabajo"]
Kf_val <- chile_sam["Empresas","Capital"]; Kg_val <- chile_sam["Gobierno","Capital"]
...
bk <- K_vals/(VA_nom_crude+pad_floor); bl <- L_vals/(VA_nom_crude+pad_floor)
...
por_cont_soc_f_v <- chile_sam["Empresas","Cont_soc"]/(chile_sam["Cont_soc","Hogares"]+pad_floor)
por_cont_soc_g_v <- chile_sam["Gobierno","Cont_soc"]/(chile_sam["Cont_soc","Hogares"]+pad_floor)
por_pres_soc_f_v <- chile_sam["Pres_soc","Empresas"]/(INGF_v_sam+pad_floor)
por_pres_soc_g_v <- chile_sam["Pres_soc","Gobierno"]/(INGG_v_sam+pad_floor)
...
TREMP_v    <- INGF_v*por_tremp_val; TRGOV_v <- INGG_v*por_trgov_val
...
SAVf_v  <- INGF_v - TREMP_v - INGF_v*por_pres_soc_f_v - FIRMTAX_v - INGF_v*por_tr_f_g_val
SAVg_v  <- INGG_v - (sum(G_m)+(INGG_v*por_trgov_val)+INGG_v*por_pres_soc_g_v)
...
sigma_q_val <- 2; sigma_t_val <- 2
rho_q_val <- (sigma_q_val-1)/sigma_q_val; rho_t_val <- (sigma_t_val+1)/sigma_t_val

Z_v  <- as.numeric(colSums(make_matrix)); names(Z_v) <- p_names
D_v  <- D_m; I_v <- I_m
E_v  <- pmax(E_m,1e-5); M_v <- pmax(M_m,1e-5)
DS_v <- Z_v - E_m; DS_v <- pmax(DS_v,1.0); DS_v["prod_otros"] <- 0
...
TD_Dom_initial <- as.numeric(D_m+G_m+I_m+V_m+rowSums(X_matrix)); names(TD_Dom_initial) <- p_names
...
io_matrix <- sweep(X_phys,2,V_phys+pad_floor,"/"); io_matrix <- pmax(io_matrix,0)
X_nom_by_sector <- colSums(sweep(X_phys,1,p_base,"*"))
VA_nom <- pmax(net_rev-X_nom_by_sector,pad_floor)
bk <- K_vals/(VA_nom+pad_floor); bl <- L_vals/(VA_nom+pad_floor)

gamma_v <- numeric(12)
for(i in 1:12) gamma_v[i] <- V_phys[i]/((K_vals[i]^bk[i])*(L_vals[i]^bl[i])+pad_floor)
...
pnoconst_names <- p_names[pnoconst_idx]; pdom_names <- p_names[pdom_idx]
pminp_names    <- p_names[pminp_idx]

par_flat <- unlist(list(
  omega=omega_val, k_total_data=sum(K_vals)+chile_sam["row","Capital"],
  l_total_data=l_total_data_val, l_force_data=l_force_data_val,
  phi=phi_val, w_pot_calib=w_pot_calib_val,
  ...
  por_cont_soc_f=por_cont_soc_f_v, por_cont_soc_g=por_cont_soc_g_v,
  por_pres_soc_f=por_pres_soc_f_v, por_pres_soc_g=por_pres_soc_g_v,
  pit=pit_val, fit=fit_val, pshdata=as.numeric(pshdata),
  ...
  damp_inv=0.5, SAVext_target=SAVext_v,
...
var_list <- list(
  INGF=INGF_v, INGG=INGG_v, FIRMTAX=FIRMTAX_v, PITAX=PITAX_v,
  ...
  LD=l_total_data_val, u_rate=u_base_val, U=U_val, lambda__CONSUMER_1=lambda_val
)
...
init_vars <- unlist(list(
  ...
  { M_init <- setNames(pmax(M_v, 1e-6), p_names); M_init["prod_adm_pub"] <- 0.0
...
cat("\n--- Residuos iniciales ---\n")
cat("¿Hay Inf?:", any(is.infinite(model@init_residual_vector)), "\n")
cat("Norma inicial:", pracma::Norm(model@init_residual_vector), "\n\n")

cat("--- Calibracion steady-state ---\n")
...
cat("\nNorma residuos FINALES:", pracma::Norm(model@residual_vector), "\n")

# Top 5 residuos
res <- model@residual_vector
top5 <- order(abs(res), decreasing=TRUE)[1:5]
cat("\n--- Top 5 residuos finales ---\n")
for(i in top5) {
  cat("Eq.", i, "(Res =", res[i], "):", model@equations[i], "\n")
}
