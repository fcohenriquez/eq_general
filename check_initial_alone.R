...
model <- set_free_par(model, c(
    omega = 2, k_total_data = flow1["Total", "Capital"], par_k_h = flow1["Hogares", "Capital"] / flow1["Total", "Capital"],
...
g_p <- 2
varlist <- list(
    p_k = 1, U = (0.2 * flow1["Prod", "Hogares"]^((2 - 1) / 2))^(2 / (2 - 1)), BTINC = flow1["Total", "Hogares"], CONT_SOC = flow1["Total", "Cont_soc"],
...
cat("\n--- ALL RESIDUALS ---\n")
print(get_residuals(model))
cat("Norm: ", Norm(model@init_residual_vector), "\n")
