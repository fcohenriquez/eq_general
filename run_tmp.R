...
vat_vals <- as.numeric(chile_sam["IVA", sam_p]) +
    as.numeric(chile_sam["Imptos_Espec", sam_p]) +
    as.numeric(chile_sam["Otros_imptos_prod", sam_p])
...
# Impuestos Indirectos Recaudados (IVA Consumo + IVA Producción + Aranceles + Otros)
# Usamos TOTAL_TAX_SAM del SAM original si es posible, o lo derivamos
vat_consumption <- sum(vat_rates * (D_vals + GG_vals + I_vals + VAR_EXIS_vals))
vat_production <- sum(t_iva_prod_rates * V_sector_vals)
tariff_total <- sum(tariff_rates * (M_market))
other_ind_taxes <- sum((t_imp_prod_rates + t_imp_espec_rates) * V_sector_vals)

TOTAL_TAX_SAM <- vat_consumption + vat_production + tariff_total + other_ind_taxes + DIR_TAX_H_val + FIT_val
...
# En un modelo CGE, SAVrow DEBE ser el déficit de cuenta corriente (BOP)
SAV_ROW_val <- (sum(M_market) + fact_row_out_k_val + fact_row_out_l_val) -
    (sum(E_market) + fact_row_in_k_val + fact_row_in_l_val)

# Verificación de Walras: (SavH + SavF + SavG + SavRow) = Inversión Total
total_inv_market <- sum((I_vals + VAR_EXIS_vals) * (1 + vat_rates))
total_savings <- SAV_H_val + SAV_F_val + SAV_G_val + SAV_ROW_val
discrepancia_sam <- total_inv_market - total_savings

cat("--- Verificación de Balance Analítico ---\n")
cat("Ahorro Externo (BOP):       ", SAV_ROW_val, "\n")
cat("Inversión Total Market:    ", total_inv_market, "\n")
cat("Ahorro Total:              ", total_savings, "\n")
cat("Discrepancia Estadística:  ", discrepancia_sam, "\n")
cat("-----------------------------------------------\n")

# Si hay discrepancia en la SAM original, la absorbemos en SAVrow para Walras
if (abs(discrepancia_sam) > 1e-5) {
    cat("Ajustando SAVrow para cerrar Walras (Discrepancia SAM detectada)\n")
    SAV_ROW_val <- SAV_ROW_val + discrepancia_sam
}
