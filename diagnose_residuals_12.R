...
# 1. Load SAM and duplicate Rmd logic
...
# NO DIAGONAL SUBTRACTION this time
...
# 3. DIAGNOSIS
print("DIAGNOSIS DE RESIDUOS:")
rv <- model@init_residual_vector
eq_names <- model@equations
res_df <- data.frame(Equation = eq_names, Residual = as.numeric(rv))
res_df <- res_df[order(-abs(res_df$Residual)), ]
print(head(res_df, 50))

cat("\nNorma 1 total:", sum(abs(rv)), "\n")
