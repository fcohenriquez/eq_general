...
Se va a modelar una economía en que los tres sectores pueden producir los 3 productos y utilizan como insumos intermedios los tres productos.
...
chile_sam <- read.csv("macro_sam_2008_simp3sect2.csv", row.names = 1, check.names = FALSE)
...
model <- make_model("cge_calib_chile_2008_simpl3sect2.gcn")
...
# El IVA se aplica sobre D, GG, I y las ventas intermedias X
# Pero en la SAM, el IVA se registra en una fila separada para cada PRODUCTO?
# Mirando la SAM de nuevo...
# Fila IVA, Columnas Prod prim, Prod manu, Prod serv.
vat_vals <- as.numeric(chile_sam["IVA", sam_p])
total_demand_market <- D_market + GG_market + I_market + rowSums(X_matrix)
vat_rates <- vat_vals / (total_demand_market - vat_vals)
vat_rates[is.nan(vat_rates)] <- 0

# Tasa de impuesto a la producción (si la hubiera, por ahora 0 o calculada)
# No hay fila de t_prod explícita, así que asumimos 0 por ahora a menos que la SAM indique lo contrario
t_prod_rates <- rep(0, 3)
...
X_vals <- X_matrix / (1 + vat_rates) # Dividimos cada fila por su respectiva tasa de IVA
```
...
model <- set_free_par(model, c(
    ...
    get_flow_values(t_prod_rates, "t_prod", p_names),
...
varlist <- list(
    ...
    FIRM_TAX = FIT_val,
    TOTAL_TAX = sum(chile_sam["IVA", sam_p]) + DIR_TAX_H_val + FIT_val + sum(t_prod_rates * V_vals),
...
beta_l_vals <- L_vals / V_vals
beta_x_vals <- sweep(X_matrix, 2, V_vals, "/")
