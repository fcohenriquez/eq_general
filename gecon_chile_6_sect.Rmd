...
# Valores iniciales dinámicos para variables agregadas
TD_initial <- as.numeric(D_m + G_m + I_m + V_m + rowSums(X_matrix) + E_m)
names(TD_initial) <- p_names
TD_Dom_initial <- as.numeric(D_m + G_m + I_m + V_m + rowSums(X_matrix))
names(TD_Dom_initial) <- p_names
pb_initial <- 1.0 * ((TD_Dom_initial * (1.0 - as.numeric(vat_r) - as.numeric(tar_r)) + E_m) / (TD_initial + 1e-9)) - (as.numeric(chile_sam["Imptos_Espec", p_names]) / (TD_initial + 1e-9))
...
init_vars <- unlist(list(
    ...
    TD_Agri = as.numeric(TD_initial["prod_Agri"]), TD_Const = as.numeric(TD_initial["prod_const"]),
    get_flow_values(as.numeric(TD_initial[pmin_names]), "TD_PMin", pmin_names),
    TD_Dom_Agri = as.numeric(TD_Dom_initial["prod_Agri"]), TD_Dom_Const = as.numeric(TD_Dom_initial["prod_const"]),
    get_flow_values(as.numeric(TD_Dom_initial[pmin_names]), "TD_Dom_PMin", pmin_names),
...
```{r solution}
# Resolver el modelo
eq_options <- list(method = "Newton", global = "gline", max_iter = 10000, tol = 3e-5)
