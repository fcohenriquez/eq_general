...
calc_pib <- function(model_obj) {
    ...
    G_val <- sum(par_vals[paste0("gg_market_data_p__", p_names)], na.rm = TRUE)
    I_val <- sum(par_vals[paste0("I_data__", p_names)], na.rm = TRUE)
...
fit_shock <- as.numeric(fit_base) * (1-0.010)
...
```{r}
# Comparación
vars_to_compare <- c("INC", "Tax", "SAVf", "INGF", "INGG", "FIRMTAX", "VAT", "pk", "w")
...
cat("--- RESULTADOS DE LA SIMULACIÓN: INCREMENTO 10% FIT ---\n")
